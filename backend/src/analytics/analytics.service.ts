...
export class AnalyticsService {
  ...
  async getMetrics() {
    ...
    const stageCounts: Record<string, number> = { Screening: 0, Interview: 0, Review: 0, Offer: 0 };
    candidates.forEach((c) => {
      if (c.stage === 'Screening' || c.stage === 'New') stageCounts.Screening++;
      else if (c.stage === 'Interview') stageCounts.Interview++;
      else if (c.stage === 'Shortlisted') stageCounts.Offer++;
      else stageCounts.Review++;
    });
