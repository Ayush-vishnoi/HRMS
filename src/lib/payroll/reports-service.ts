...
export function generatePayrollRegister(items: any[]): PayrollReportRow[] {
...
export function generatePfEcrReport(items: any[]) {
  return items.map((item) => {
...
export function generateEsicReturnReport(items: any[]) {
  return items
...
export function generateDepartmentCostingReport(items: any[]) {
  const map = new Map<string, { count: number; gross: number; statutory: number; totalCost: number }>();
