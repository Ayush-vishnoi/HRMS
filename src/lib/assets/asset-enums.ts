...
const REQUEST_TYPE_TO_PRISMA: Record<string, string> = { 'New Asset': 'NewAsset', 'Issue Report': 'IssueReport' };
const REQUEST_TYPE_FROM_PRISMA: Record<string, string> = { NewAsset: 'New Asset', IssueReport: 'Issue Report' };

export const toPrismaAssetCategory = (value: string): AssetCategory =>
  ((CATEGORY_TO_PRISMA[value] ?? value) as AssetCategory);
export const fromPrismaAssetCategory = (value: string): string => CATEGORY_FROM_PRISMA[value] ?? value;

export const toPrismaAssetCondition = (value: string): AssetCondition =>
  ((CONDITION_TO_PRISMA[value] ?? value) as AssetCondition);
export const fromPrismaAssetCondition = (value: string): string => CONDITION_FROM_PRISMA[value] ?? value;

export const toPrismaRequestType = (value: string): AssetRequestType =>
  ((REQUEST_TYPE_TO_PRISMA[value] ?? value) as AssetRequestType);
export const fromPrismaRequestType = (value: string): string => REQUEST_TYPE_FROM_PRISMA[value] ?? value;
