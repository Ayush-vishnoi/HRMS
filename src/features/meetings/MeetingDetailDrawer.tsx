...
export function MeetingDetailDrawer({ meeting, onClose, onEdit }: { meeting: Meeting | null; onClose: () => void; onEdit: (meeting: Meeting) => void }) {
  ...
  const canManage = meeting.organizer.id === currentUser.id || currentUser.userRole === 'manager' || currentUser.userRole === 'admin';
  ...
  return <div className="drawer-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><aside className="drawer-panel" role="dialog" aria-modal="true" aria-labelledby="meeting-title"><header className="drawer-header"><div><span className={`type-badge ${meeting.type === 'ORG_EVENT' ? 'type-org' : 'type-team'}`}>{meeting.type === 'ORG_EVENT' ? 'Org event' : 'Team meeting'}</span><h2 id="meeting-title" className="mt-2 text-xl font-semibold text-[#f0f6fc]">{meeting.title}</h2></div><button onClick={onClose} className="icon-button" aria-label="Close meeting details"><X className="h-4 w-4" /></button></header>
    <div className="drawer-body space-y-6"><div className="detail-grid"><div><CalendarDays /><span><b>{meeting.allDay ? 'All-day event' : dateTime(meeting.startsAt)}</b>{!meeting.allDay && <small>Ends {dateTime(meeting.endsAt)}</small>}</span></div>{meeting.location && <div><MapPin /><span><b>{meeting.location}</b><small>Location</small></span></div>}{meeting.videoLink && <div><Video /><span><a href={meeting.videoLink} target="_blank" rel="noreferrer" className="text-[#58a6ff]">Join video meeting <ExternalLink className="inline h-3 w-3" /></a><small>Video conference</small></span></div>}<div><Repeat /><span><b className="capitalize">{meeting.recurrence.toLowerCase()}</b><small>Recurrence</small></span></div></div>
      ...
      <section><div className="flex items-center justify-between"><h3 className="section-label">Attendees</h3><span className="text-xs text-[#8b949e]">{meeting.attendees.length} invited</span></div>{meeting.attendees.length === 0 ? <p className="mt-2 text-xs text-[#8b949e]">Company-wide event. No individual invitation list.</p> : <div className="mt-2 divide-y divide-[#30363d] border border-[#30363d]">{meeting.attendees.map((attendee) => <div className="person-row px-3 py-2" key={attendee.id}><img src={attendee.avatarUrl} alt="" /><span><b>{attendee.name}</b><small>{attendee.department}</small></span><span className={`rsvp-badge rsvp-${attendee.rsvp?.toLowerCase()}`}>{attendee.rsvp}</span></div>)}</div>}</section>
      ...
      {meeting.status === 'CANCELLED' && <div className="border border-[#6e7681] bg-[#21262d] p-3 text-sm text-[#8b949e]">This meeting has been cancelled.</div>}
