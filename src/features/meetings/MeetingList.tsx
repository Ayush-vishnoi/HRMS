...
export function MeetingList({ onSelect, onSchedule }: { onSelect: (meeting: Meeting) => void; onSchedule: () => void }) {
  ...
  return <section className="space-y-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="eyebrow"><List className="h-3.5 w-3.5" /> Meetings</p><h1 className="page-title">Meeting list</h1><p className="page-subtitle">Search and review scheduled, past, and cancelled meetings.</p></div><div className="flex gap-2"><Link href="/meetings/calendar" className="toolbar-button"><CalendarDays className="h-3.5 w-3.5" /> Calendar</Link><button onClick={onSchedule} className="primary-button"><Plus className="h-4 w-4" /> Schedule</button></div></div>
